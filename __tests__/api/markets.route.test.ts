...
const mockGetClientIdentifier = getClientIdentifier as jest.MockedFunction<
  typeof getClientIdentifier
>;
...
describe('api/markets', () => {
  ...
  describe('Success (200)', () => {
    it('should return 200 status with data for required parameters', async () => {
      ...
      expect(mockFetchMarketData).toHaveBeenCalledWith(
        expect.objectContaining({ vs_currency: 'usd' }),
      );
    });
    ...
    it('should validate and pass all query parameters to fetchMarketData', async () => {
      ...
      const req = new Request(
        'http://localhost/api/markets?vs_currency=eur&per_page=50&page=2&order=market_cap_asc',
      );
      const res = await GET(req as any);
  ...
  describe('Parameter Validation (400)', () => {
    it('should return 400 when per_page is out of range (too low)', async () => {
      ...
      const req = new Request(
        'http://localhost/api/markets?vs_currency=eur&per_page=50&page=0&order=market_cap_asc',
      );
      const res = await GET(req as any);
    ...
    it('should return 400 when per_page is out of range (too high)', async () => {
      ...
      const req = new Request(
        'http://localhost/api/markets?vs_currency=eur&per_page=50&page=9999&order=market_cap_asc',
      );
      const res = await GET(req as any);
    ...
    it('should return 400 when price_change_percentage has duplicates', async () => {
      ...
      const req = new Request(
        'http://localhost/api/markets?vs_currency=usd&price_change_percentage=1h,1h,24h',
      );
      const res = await GET(req as any);
